...
const DBConnections = {
  init() {
    ...
    document.getElementById("cops-lookup-btn")?.addEventListener("click", async () => {
      const project = document.getElementById("cops-project")?.value.trim();
      if (!project) { Toast.error("Enter a project name."); return; }
      const r = await postJSON("/db-connections/cops-lookup", { project });
      if (r.error) { Toast.error(r.error); return; }
      const info = r.info;
      document.getElementById("cops-result").innerHTML = `
        <div class="card" style="margin-top:12px">
          <div class="card-title">Found: ${escHtml(project)}</div>
          <table class="data-table">
            ${Object.entries(info).map(([k,v])=>`<tr><td style="font-weight:600;width:40%">${escHtml(k)}</td><td>${escHtml(v||'—')}</td></tr>`).join("")}
          </table>
        </div>`;
    });
  },
...
const AskDBA = {
  ...
  async discover() {
    ...
    document.getElementById("cops-result-panel").innerHTML = `
      <div class="card" style="border-left:4px solid ${r.db_type==='oracle'?'#F97316':'#3B82F6'}">
        ...
        <input type="hidden" id="cops-project" value="${escHtml(r.project)}">
        <input type="hidden" id="cops-db-type" value="${escHtml(r.db_type)}">
        <input type="hidden" id="cops-host" value="${escHtml(r.host)}">
        <input type="hidden" id="cops-port" value="${escHtml(r.port)}">
        <input type="hidden" id="cops-db-name" value="${escHtml(r.db_name)}">
        <input type="hidden" id="cops-service" value="${escHtml(r.service)}">
        <input type="hidden" id="cops-user" value="${escHtml(r.db_user)}">
        <input type="hidden" id="cops-question" value="${escHtml(r.question)}">
  ...
  async connect() {
    const payload = {
      project:  document.getElementById("cops-project")?.value,
      db_type:  document.getElementById("cops-db-type")?.value,
      host:     document.getElementById("cops-host")?.value,
      port:     document.getElementById("cops-port")?.value,
      db_name:  document.getElementById("cops-db-name")?.value,
      service:  document.getElementById("cops-service")?.value,
      db_user:  document.getElementById("cops-user")?.value,
